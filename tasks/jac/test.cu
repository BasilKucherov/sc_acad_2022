...
int main(int an, char **as)
{
    ...
    init_a_gpu<<<dim3((M + 7) / 8, (N + 7) / 8, (K + 7) / 8), dim3(8, 8, 8)>>>(dA, M, N, K);
...
#define c(i,j,k) c[((i)*nn+(j))*kk+(k)]

double jac(double *a, double* dA, int mm, int nn, int kk, int itmax, double maxeps)
{
    size_t x_dim = mm, y_dim = nn, z_dim = kk;
    double *hdA;
    SAFE_CALL (cudaMallocHost ((void**) &hdA, M*N*K * sizeof(double)));
    ...
    c = (double*) malloc(mm*nn*kk*sizeof(double));

    for (int it = 1; it <= itmax - 1; it++)
    {
        SAFE_CALL (cudaMemset(dC, 0, dC_len * sizeof(double)));
        memset(c, 0,dC_len * sizeof(double));
        SAFE_CALL (cudaDeviceSynchronize());

        printf("------ ITERATION #%d ------\n", it);
        SAFE_CALL (cudaMemcpy ( hdA, dA, M*N*K*sizeof(double), cudaMemcpyDeviceToHost ));
        // print_arr(hdA, N*M*K);
        printf("\nA arrs:\n");
        compare_arrs(hdA, a, N*M*K);

        SAFE_CALL (cudaMemcpy ( hdA, dB, M*N*K*sizeof(double), cudaMemcpyDeviceToHost ));
        // print_arr(hdA, N*M*K);
        printf("\nB arrs:\n");
        compare_arrs(hdA, b, N*M*K);
        ...
        jac_calc_b_gpu<<<dim3((x_dim + 7) / 8, (y_dim + 7) / 8, (z_dim + 7) / 8), dim3(8, 8, 8)>>>(dA, dB, x_dim, y_dim, z_dim);
        SAFE_CALL (cudaDeviceSynchronize());
        ...
        calc_c_gpu<<<dim3((x_dim + 7) / 8, (y_dim + 7) / 8, (z_dim + 7) / 8), dim3(8, 8, 8)>>>(dA, dB, dC, x_dim, y_dim, z_dim);
        
        SAFE_CALL (cudaMemcpy ( hdA, dC, M*N*K*sizeof(double), cudaMemcpyDeviceToHost ));
        // print_arr(hdA, N*M*K);
        printf("\nC arrs:\n");
        compare_arrs(hdA, c, N*M*K);
        ...
        SAFE_CALL (cudaMemcpy ( &gpu_eps, dC, sizeof(double), cudaMemcpyDeviceToHost ));
        ...
        if (TRACE && it%TRACE == 0)
            printf("\nIT=%d eps=%.4g gpu_eps=%.4g\t", it, eps, gpu_eps);
        if (eps < maxeps) 
            break;
