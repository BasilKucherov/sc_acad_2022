...
int main(void)
{
    ...
    double *hdC;
    cudaMallocHost ((void**) &hdC, ARR_SIZE);
    ...
    for (int i = 0; i < CUDA_STREAM_NUMBER; i++ ) {
        cudaMemcpyAsync ( hdC + i * arr_len_per_stream, dC + i * arr_len_per_stream, arr_size_per_stream, cudaMemcpyDeviceToHost, streams[i]);
    }

   // Синхронизация CUDA-streams
   cudaDeviceSynchronize ();
   // Уничтожение CUDA-streams
    for (int i = 0; i < CUDA_STREAM_NUMBER; i++ ) cudaStreamDestroy ( streams[i] );
    ...
    printf("Acceleration: %.1lfx\n", calc_cpu_time / calc_gpu_time);
